...
# Category mapping (as they appear in your template)
TEMPLATE_CATEGORIES = [
    'wasser/heizung',
...
def get_category_column(position, comment):
    ...
    # Tax advisor - check comment if position doesn't match
    if 'steuerberater' in comment_lower:
...
def generate_excel(monthly_data, location='Hollgasse_1_54', year=2026, output_file=None):
    ...
    # Row 1: Title (merged across columns A-P) with yellow background
    ...
    # Column mapping
    category_columns = {
        'miete': 5,          # E - mieteinkommen
        'hausverwaltung': 6,  # F
        'haushaltsversicherung': 7,  # G
        'strom': 8,          # H
        'wasser': 9,         # I - wasser/heizung
        'av': 10,            # J
        'kleinmaterial': 11, # K
        'internet': 12,      # L
        'klimaanlage': 13,   # M
    ...
    for month in range(1, 13):
        ...
        for category in TEMPLATE_CATEGORIES:
            ...
            ws.cell(row=current_row, column=3).alignment = normal_alignment
            ...
            if category in categories_in_month:
                ...
                for entry in data['entries']:
                    ...
                    # Column C: Description (already set above, but update with actual description)
